...
from box import Box
from src.model.GraphNets import GraphPrecond
from src.utils.data import U1DDataset
...
def main(configs):
    ...
    logname = "_".join(["model"] + model_config + ["train"] + train_config)
    ...
    train_data = U1DDataset(configs.data.path, mode="train")
    valid_data = U1DDataset(configs.data.path, mode="val")
    ...
    model = GraphPrecond(**configs.model, key=jax.random.PRNGKey(0))
...
if __name__ == "__main__":
    configs = Box.from_yaml(filename="./configs/graph_config.yaml")
    main(configs)
